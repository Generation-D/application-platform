...
def main():
    ...
    with open(f'{rootdir}/completed.csv', 'r') as file:
        applicants_csv = file.read().split('\n')
    available_files = os.listdir(rootdir)
    open(f'{rootdir}/completed_share_links.csv', 'w').close()

    for row in applicants_csv:
        file_name = row.split(',')[-1]
        fullname = os.path.join(rootdir, file_name)
        if os.path.isfile(fullname) and fullname.endswith('.pdf'):
            if yesno('Upload %s' % file_name, True, args):
                upload(dbx, fullname, folder, '', file_name, overwrite=True)
                shared_url = create_shared_link(dbx, folder, '', file_name)
                print(shared_url)
                # append the shared url to the csv
                with open(f'{rootdir}/completed_share_links.csv', 'a') as file:
                    file.write(f'{row},{shared_url}\n')
    dbx.close()
...
def create_shared_link(dbx, folder, subfolder, name):
    """Download a file.
...
def upload(dbx, fullname, folder, subfolder, name, overwrite=False):
    ...
    with stopwatch('upload %d bytes' % len(data)):
        try:
            res = dbx.files_upload(data, path, mode, client_modified=datetime.datetime(*time.gmtime(mtime)[:6]),
                                   mute=True)
        except dropbox.exceptions.ApiError as err:
            print('*** API error', err)
            return None
