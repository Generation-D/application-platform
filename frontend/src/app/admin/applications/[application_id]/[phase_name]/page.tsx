...
export default async function Application({
    ...
    params: Promise<{ application_id: string, phase_name: string }>;
}>) {
    const { application_id, phase_name } = await params

    const phaseData = await cached_fetch_phase_by_name(phase_name);
    ...
    const phase_answers = await fetchAllAnswersOfApplication(application_id);
    ...
    return (
        <span className="w-full">
            <div className="flex flex-col items-start justify-between space-y-4">
                <Apl_Header />
                <OverviewButton slug={`admin/applications/${application_id}`} text="<- Appliaction Overview"/>
                <div className="w-full">
                    <h2 className="p-4 rounded text-secondary">
                        <b>
                            Bewerbung {application_id}
                        </b>
                        <br />
                        <b> 
